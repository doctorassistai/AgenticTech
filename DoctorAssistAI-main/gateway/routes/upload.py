...
class ECGMODEL(BaseModel):
    ...
    file_type: Optional[str] = Field(
        default=None, description="Type of document (pdf/mp4/mov/avi)"
    )
...
async def upload_dicom(
    ...
    if document_type not in allowed_types:
        return JSONResponse(
            status_code=400, content={"error": "Invalid document type."}
        )
    ...
    try:
        ...
        async with httpx.AsyncClient(
            auth=(ORTHANC_USER, ORTHANC_PASSWORD), timeout=None
        ) as client:
            response = await client.post(
                ORTHANC_STOW, content=multipart_body, headers=headers
            )
        ...
        if response.status_code not in (200, 202):
            return JSONResponse(
                status_code=500,
                content={
                    "error": "Orthanc STOW-RS upload failed",
        ...
        for f in files:
            ...
            logger.info(
                f"📁 Uploaded file {f.filename} for patient {patient_id} under study {study_uid}"
            )
        # Convert dict to list for MongoDB push
        studies_to_insert = list(study_dict.values())
        logger.info(
            f"🗂 Preparing to insert {len(studies_to_insert)} studies for patient {patient_id}"
        )
        ...
        dicom_collection.update_one(
            {
                "patient_id": patient_id,
                f"documents.{document_type}": {"$exists": False},
            },
        ...
        for study in studies_to_insert:
            ...
            dicom_collection.update_one(
                {
                    "patient_id": patient_id,
                    f"documents.{document_type}.study_uid": {"$ne": study["study_uid"]},
                },
...
def get_dicom_studies(patient_id: str):
    ...
    try:
        ...
        if not studies:
            return JSONResponse(
                status_code=404,
                content={"error": "No studies found for this patient_id."},
            )

        return JSONResponse(
            status_code=200, content={"patient_id": patient_id, "studies": studies}
        )

    except Exception:
        logger.error(traceback.format_exc())
        return JSONResponse(
            status_code=500, content={"error": "Failed to retrieve studies."}
        )
...
async def get_patient_documents(
    ...
    document_type: Optional[str] = Query(
        None, description="Optional document type filter (MRI, X-ray, etc.)"
    ),
):
    """
    ...
    try:
        ...
        patient_record = dicom_collection.find_one(
            {"patient_id": patient_id}, {"_id": 0}
        )
        if not patient_record:
            return JSONResponse(
                status_code=404, content={"error": "Patient not found."}
            )
            ...
            return [
                {
                    ...
                    "uploaded_at": (
                        s.get("uploaded_at").isoformat()
                        if isinstance(s.get("uploaded_at"), datetime)
                        else s.get("uploaded_at")
                    ),
                    "files": s.get("files", []),
                }
                for s in studies
        ...
        if document_type:
            ...
            serialized_documents = {
                doc_type: serialize_studies(studies)
                for doc_type, studies in documents.items()
            }
            return JSONResponse(
                status_code=200,
                content={"patient_id": patient_id, "documents": serialized_documents},
            )
...
async def upload_diabetic_foot_pdf(
    ...
    try:
        ...
        unique_filename = (
            f"{patient_id}_{doctor_id}_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}.pdf"
        )
...
async def get_diabetic_foot_pdfs(patient_id: str):
    ...
    try:
        ...
        if not pdfs:
            raise HTTPException(
                status_code=404, detail=f"No PDFs found for patient_id={patient_id}"
            )
        ...
        return JSONResponse(
            status_code=200,
            content={"status": "success", "count": len(pdfs), "data": pdfs},
        )
...
async def upload_echo_files(
    ...
    if len(files) != len(timestamps):
        raise HTTPException(
            status_code=400, detail="Number of timestamps must match number of files."
        )
    ...
    try:
        ...
        for idx, file in enumerate(files):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid timestamp format for file {file.filename}",
                )
            ...
            if patient_doc:
                await echo_collection.update_one(
                    {"patient_id": patient_id}, {"$push": {"echo_files": echo_entry}}
                )
            else:
                await echo_collection.insert_one(
                    {"patient_id": patient_id, "echo_files": [echo_entry]}
                )
            ...
            try:
                ...
                logger.error(
                    f"❌ PanEcho/LLM failed for {file.filename}: {e}", exc_info=True
                )
...
async def get_echo_files(patient_id: str):
    ...
    try:
        ...
        if (
            not patient_doc
            or "echo_files" not in patient_doc
            or not patient_doc["echo_files"]
        ):
            raise HTTPException(
                status_code=404,
        ...
        for file in patient_doc["echo_files"]:
            ...
            if "timestamp" in file_copy and isinstance(
                file_copy["timestamp"], datetime
            ):
                file_copy["timestamp"] = file_copy["timestamp"].isoformat()
            if "created_at" in file_copy and isinstance(
                file_copy["created_at"], datetime
            ):
        ...
        logger.error(traceback.format_exc())
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve echo files: {e}"
        )
...
"""
        ...
        logger.error(traceback.format_exc())
        raise HTTPException(
            status_code=500, detail=f"LLM echo interpretation failed: {e}"
        )
...
async def upload_pathology(
    ...
    if pathology_type not in valid_types:
        raise HTTPException(
            status_code=400,
            detail="Invalid pathology type. Allowed: cyto, biopsy, histo",
        )
    ...
    logger.info(
        f"📥 upload_pathology filename={filename}, type={pathology_type}, patient_id={patient_id}"
    )
    ...
    try:
        ...
        await pathology_collection.update_one(
            {
                "patient_id": patient_id,
                f"pathology.{pathology_type}": {"$exists": False},
            },
...
async def get_pathology_by_patient(patient_id: str):
    ...
    try:
        record = await pathology_collection.find_one(
            {"patient_id": patient_id}, {"_id": 0}  # hide MongoDB ObjectId
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve pathology: {e}"
        )


@router.get("/get-pathology/{patient_id}/{type}")
async def get_pathology(patient_id: str, type: str):
    """
