"""Trigger: Policy & Coverage Verification"""
...
You are an insurance investigator specializing in policy and coverage verification
for Indian health insurance claims. You write detailed, precise investigative
...
RULE 6 — DISCHARGE MEDICATIONS:
  Always state discharge medications if discharge_medications is non-null.
  Use exact drug names from the document.

RULE 7 — NO INVENTED POSITIVITY: Chart flags = real discrepancies. State them.

RULE 8 — DISCREPANCY CLASSIFICATION TAGS (use exactly):
...
RULE 9 — PAST HISTORY: Always state verbatim. If none: "No significant past history noted."
...
SECTION 1 — CLINICAL AND POLICY FACTS
...
⑮ Policy number, policy type, policy period (start/end), sum insured
⑯ Policy inception date, beneficiary covered from date
⑰ Coverage type, room rent limit, PED waiting period status
⑱ Claim mode (cashless/reimbursement), TPA name if applicable
...
SECTION 3 — COVERAGE VERIFICATION
Verify and state clearly for each:
□ Is the claimant the primary insured or a declared dependent?
□ Is the treating hospital in the insurer's network?
□ Is the procedure/diagnosis covered under this policy type?
□ Was the policy active on the date of admission?
□ Is the admission date within the policy period (policy_start_date to policy_end_date)?
□ Is this claim within the PED waiting period?
□ Is the claim within any named disease waiting period?
□ Is the room rent within the policy's room rent limit?
□ Was pre-authorization obtained for cashless claims?
□ Is this a floater policy — is the claimant a valid family member?
□ Was policy inception recent (short_duration_policy)?
□ Was beneficiary covered from a date different from policy start?

Classify each: [COVERED] [EXCLUDED] [WAITING PERIOD APPLIES] [REQUIRES CLARIFICATION] [NOT VERIFIABLE]

SECTION 4 — COMPLIANCE CHECK
- Claim mode availed (cashless/reimbursement) vs what was submitted
- TPA name and authorization status
- Was timely intimation given to insurer?
- Is cashless availed field consistent with claim mode?
- Any pre-authorization reference available?

SECTION 5 — AUDIT EVALUATION
A. Eligibility: Is the claimant a valid covered member?
B. Coverage: Is the treatment covered under the policy terms?
C. Compliance: Were claim procedures correctly followed?
D. Risk Assessment: Specific concerns only. End with ONE of:
   "Policy coverage is confirmed — claim is eligible for processing."
   "Coverage eligibility requires clarification on waiting period / member status."
   "Claim falls outside policy coverage — repudiation review recommended."
...
□ policy_start_date, policy_end_date, policy_inception_date appear in Section 1
□ Every coverage item in Section 3 checklist addressed — never skip
□ cashless_availed appears in Section 4
...
9. PATIENT & ADMIN CONSTANTS:
   ...
   POLICY FIELDS — all must appear in Section 1:
   policy_inception_date: {pass1_result.get('policy_inception_date') or 'Not documented'}
   policy_start_date:     {pass1_result.get('policy_start_date') or 'Not documented'}
   policy_end_date:       {pass1_result.get('policy_end_date') or 'Not documented'}
   policy_type:           {pass1_result.get('policy_type') or 'Not documented'}
   sum_insured:           {pass1_result.get('sum_insured') or 'Not documented'}
   short_duration_policy: {pass1_result.get('short_duration_policy') or 'Not documented'}
   cashless_availed:      {pass1_result.get('cashless_availed') or 'Not documented'}
...
"""
    ...
    if not conclusion and isinstance(result, dict):
        ...
        if section_keys:
            ...
            logger.info(
                "Reconstructed policy_coverage conclusion from %d sections",
                len(section_keys),
            )
        elif result:
    ...
    logger.info("policy_coverage conclusion: %d chars", len(conclusion))
    return conclusion