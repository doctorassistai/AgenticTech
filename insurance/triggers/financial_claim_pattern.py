"""Trigger: Financial & Claim Pattern Risk"""
...
You are an insurance fraud analyst specializing in financial and claim pattern risk,
...
Multi-section financial risk audit report. Third person, past tense.
Use \\n\\n between sections.
...
SECTION 1 — CLINICAL AND FINANCIAL FACTS
...
- Claimed amount, sum insured, bill amount (gross), discount, room tariff,
  full bill breakdown line by line, mode of payment
- Policy type, policy age (short_duration_policy)
- Previous claims (claim_frequency, previous_claims)
...
SECTION 3 — FINANCIAL PATTERN ANALYSIS
Verify and state clearly for each:
□ Claimed amount vs sum insured ratio — is it disproportionate?
□ Is this a short-duration policy (< 1 year)? (short_duration_policy)
□ Previous claims in this policy or prior policies (previous_claims, claim_frequency)
□ Is bill amount suspiciously round or does it exactly match sum insured?
□ Are medicine/lab charges disproportionate to diagnosis severity?
□ Is ICU billed — is it supported by ICU documentation?
□ Is there billing mismatch between clinical records and final bill?
□ Is the hospital known for inflated billing patterns?
□ Was payment made in cash — unusual given the amount?
□ Is room type billed consistent with room type actually occupied?
□ Are pharmacy charges consistent with the medication chart?

Classify each: [HIGH RISK] [MODERATE RISK] [LOW RISK] [BILLING INFLATION] [PATTERN FRAUD]

SECTION 4 — BILLING AUDIT
- Bill breakdown vs clinical records — match or mismatch for each line item
- Pharmacy charges vs medication chart drugs
- Lab charges vs lab reports available
- ICU charges vs ICU documentation present/absent
- Surgeon/anaesthetist charges vs OT records
- Discount pattern — was discount offered to avoid scrutiny?
- Room tariff per day vs billed room type

SECTION 5 — AUDIT EVALUATION
A. Clinical Genuinity: Does clinical severity justify the cost?
B. Documentation Quality: Are billing documents consistent with clinical records?
C. Financial Risk: Is billing inflated, duplicated, or disproportionate?
D. Fraud Pattern Assessment: Specific concerns only. End with ONE of:
   "Billing appears inflated — claim requires detailed line-item scrutiny."
   "Financial pattern shows moderate risk — specific items need clarification."
   "No significant financial risk indicators found — claim appears proportionate."

SECTION 6 — CONCLUSION
Summary → Financial risk determination → Field officer opinion (only if name and opinion non-null) →
...
□ claimed_amount, sum_insured, bill_amount, bill_breakdown all appear in Section 1
...
□ previous_claims, claim_frequency, short_duration_policy appear in Section 3
□ Every pharmacy/lab/ICU/OT line item cross-checked in Section 4
...
9. FINANCIAL / CLAIM PATTERN CONSTANTS:
   ...
   Mode of payment:          {pass1_result.get('mode_of_payment') or 'Not documented'}
   Room type billed vs occupied: {pass1_result.get('room_type_billed') or 'Not documented'} / {pass1_result.get('room_type_occupied') or 'Not documented'}
   ICU billed:                {pass1_result.get('icu_billed') or 'Not documented'}
   ICU documentation present: {pass1_result.get('icu_documentation_present') or 'Not documented'}
...
"""
    ...
    if not conclusion and isinstance(result, dict):
        ...
        if section_keys:
            ...
            logger.info(
                "Reconstructed financial_claim_pattern conclusion from %d sections",
                len(section_keys),
            )
        elif result:
    ...
    if preprocessed.get("_is_genuine") and conclusion:
        ...
        if "claim seems to be suspected" in lower or "not recommended for settlement" in lower:
            conclusion = re.sub(
                ...
                "As per the verification, the financial pattern findings confirm the claim is found to be Genuine.",
                conclusion,
                count=1,
            )
    ...
    logger.info("financial_claim_pattern conclusion: %d chars", len(conclusion))
    return conclusion