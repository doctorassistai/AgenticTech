...
Example: "Bill breakdown — Registration - Rs.500/-, Bed Charges - Rs.6000/-, ..."
If you cannot fit them in one line, use a bullet list.
Never write "etc." or "…".
"""
...
RULE 3 — VITALS:
  For MEDICAL cases: vitals_on_admission must appear verbatim with ALL markers.
  Format: "O/E T-103F, PR-126/min, BP-90/60 mmHg, SpO2-88% RA, RR-23/min, GRBS-120mg%"
  Include rr_on_admission if non-null. Never write "stable" or "monitored".
  For SURGICAL cases: vitals on admission are typically brief (BP, PR, SpO2, GRBS, Temp).
  State them exactly as documented. Never omit GRBS if present.
  DISCHARGE VITALS: Use vitals_at_discharge EXACTLY as extracted.
  FORBIDDEN: Do not alter, round, or recompute any discharge vital value.
...
RULE 4 — ALL MEDICATIONS:
  ...
  For surgical cases it is acceptable to use abbreviated drug names matching
  the document (e.g. "Inj Mero" instead of "Meropenem") followed by "etc."
  if the list is long. Never invent drugs not in the document.
...
RULE 5 — OPD HISTORY BEFORE ADMISSION:
  ...
  Example: "Patient had taken treatment on OPD basis on 07/03/2026 but found no relief.
  Hence got admitted for further management."

RULE 6 — SURGICAL CASES — MANDATORY ELEMENTS:
  If case is surgical (operation_record_attached, anaesthesia_type, or OT in bill):
  a) State surgery date verbatim: "underwent [procedure] under [anaesthesia] on [date]"
  b) State: "Operation record is attached." if operation_record_attached is non-null.
  c) State postoperative period: "Post-operative period was noted as uneventful."
     if postoperative_period is non-null or "uneventful" appears in document.
  d) State OT register status: "OT register verified and attached." if ot_register_attached = YES.
...
RULE 10 — PAST HISTORY: Always state verbatim. If none: "No significant past history noted."
  For surgical cases "Nothing Significant" is the verbatim phrase if that is what appears.

RULE 11 — BILL DETAILS:
  ...
  - Room tariff per day stated explicitly: "Room tariff — Rs.3,000/- per day (SPL ward)"
  - Bill breakdown: all line items (bed charges, professional charges, nursing, pharmacy,
    surgeon charges, OT charges, anaesthesia charges, lab, registration, others)
...
RULE 12 — PAGE REFERENCES: Include -XX/52- format wherever present in FORENSIC AUDIT FACTS.

RULE 13 — REGISTERS — WRITE ONLY WHAT IS CONFIRMED:
  For each register, use the BINDING INSTRUCTION 5 flags.
...
RULE 14 — DISCREPANCIES VERBATIM BLOCK:
  If discrepancies_verbatim is non-null: reproduce the ENTIRE block EXACTLY as written,
  including all page references like -14/52-. Do NOT paraphrase. Do NOT reorder.
  Then append auto-detected flags below it (do not duplicate what is already verbatim).

RULE 15 — AUTO-DETECT DISCREPANCIES (add these if not already in verbatim block):
  ...
  SpO2 values > 100% anywhere in vitals chart
    → [SUSPICIOUS] Vitals chart contains physiologically impossible SpO2 readings (>100%)

RULE 16 — POSITIVE EVIDENCE SUPPRESSES FLAGS:
  ...
  reg_certificate_attached is non-null → do NOT flag reg cert as missing
  tariff_attached = YES → do NOT flag tariff as missing

RULE 17 — GENUINE CLAIM — CONCLUSION FORMAT:
  When field_officer_hospital_opinion contains "Genuine" / "genuine" AND
  ip_register_collected = YES AND no CRITICAL discrepancies exist:
  Section 2 opens with: "No major discrepancies noted."
  Section 4 (CONCLUSION) must follow this structure:
    ① "Investigated and verified by [agency], physical verification done." (if agency non-null)
    ② Condensed clinical summary (2-3 sentences max)
    ③ Register verification summary
    ④ Field officer opinion verbatim (if non-null)
    ⑤ RECOMMENDATION paragraph (labelled separately):
       "As per the verification, admission is verified and claim found to be Genuine."
       OR use final_verdict_verbatim if non-null.

RULE 18 — SUSPECTED CLAIM — CONCLUSION FORMAT:
  When significant discrepancies exist:
  Section 4 concludes with:
  "Hence based on the above discrepancies, the claim seems to be suspected."
  OR use final_verdict_verbatim if non-null.

RULE 19 — TREATING DOCTOR — STANDALONE LINE:
...
RULE 20 — NO FILLER: Every sentence introduces a new fact. No restating.

RULE 21 — PATHOLOGIST:
  If pathologist_name is non-null, state after treating doctor line:
  "Pathologist — [name] ([designation])."
...
⑩ Surgical cases: procedure, anaesthesia, operation record, post-op period
...
Append auto-detected flags (Rule 15) not already in verbatim.
...
D. Fraud Suspicion Assessment

SECTION 4 — CONCLUSION
Follow RULE 17 for genuine claims. Follow RULE 18 for suspected claims.
Always end with either final_verdict_verbatim (if non-null) or the appropriate verdict phrase.
For genuine claims, label the final paragraph:
"RECOMMENDATION
As per the verification, admission is verified and claim found to be Genuine."
'''
...
async def run(
    ...
    if case_type == "SURGICAL":
        drug_rule = (
            ...
            "Use the abbreviated names exactly as they appear in the chart. "
...
□ Section 4 verdict = {preprocessed['verdict_override']}
...
9. PATIENT & ADMIN CONSTANTS:
   ...
   Pathologist:        {pass1_result.get('pathologist_name') or 'Not documented'}, {pass1_result.get('pathologist_designation') or ''}
...
"""
    ...
    if not conclusion and isinstance(result, dict):
        ...
        if section_keys:
            ...
            logger.info(
                "Reconstructed claim_genuinity conclusion from %d sections",
                len(section_keys),
            )
        elif result:
    ...
    logger.info("claim_genuinity conclusion: %d chars", len(conclusion))
    return conclusion