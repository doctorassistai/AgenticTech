...
import {
  ...
  Building, Clipboard, BarChart3, Settings, Bell, Search, ChevronRight
} from "lucide-react";

const API_BASE_URL = import.meta.env.VITE_BACKEND_URL;

/* ─── THEME TOKENS (matching RegisterDoctor) ─── */
const T = {
  bg: "#ffffff",
  bgAlt: "#fafafa",
  bgTert: "#f5f5f5",
...
/* ─── STYLES (exact match to RegisterDoctor) ─── */
const S = {
  layout: {
    display: "flex",
    minHeight: "100vh",
...
function NurseRegister() {
  ...
  const fetchDoctors = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}hms/users/data/get_doctors_list/${hospitalId}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.doctor_id) {
      setMessage("Registration failed: Please select an assigned doctor");
      return;
    }
    setLoading(true);
    ...
    try {
      const response = await fetch(`${API_BASE_URL}hms/users/doctors/nurseadd`, {
        method: "POST",
  ...
  const selectedDoctor = doctors.find((d) => d.sys_user_id === formData.doctor_id);
  ...
  const navSections = [
      ...
      items: [
        {
          label: "Dashboard",
          icon: <Home size={14} />,
          action: () => navigate(`/hospital-dashboard?hospital_id=${hospitalId}`),
        },
    ...
    },
    {
      ...
      items: [
        ...
        { label: "Add Nurse", icon: <UserPlus size={14} />, action: handleAddNurse, active: true },
  ...
  return (
    <div style={S.layout}>
      <style>
        {`
          ...
          * { box-sizing: border-box; }
          ...
          .h-opt:hover { background: ${T.bgAlt} !important; }
      ...
      <main style={S.main}>
        <div style={S.topBar}>
          <div>
            <p style={S.topBarTitle}>Register New Nurse</p>
            <p style={S.topBarSub}>Add a nursing professional to your healthcare team</p>
          </div>
        ...
        <div style={S.body}>
          ...
          <h1 style={S.pageTitle}>Nurse Registration</h1>
          ...
          <div style={S.formContainer}>
            <form onSubmit={handleSubmit}>
              <div style={S.formInner}>
                <div style={S.grid2}>
                          ...
                          <div style={{ maxHeight: "260px", overflowY: "auto" }}>
                            {countryCodes.map((country) => (
                              <button
                                ...
                                style={S.dropdownOption(formData.country_code === country.code)}
                              >
                  ...
                  <div style={S.field}>
                    ...
                    <input
                      ...
                      onBlur={(e) => Object.assign(e.target.style, inputBlurStyle)}
                    />
                    <span style={{ fontSize: "0.6rem", color: T.textMuted, marginTop: "2px" }}>
                  ...
                  <div style={S.field}>
                    ...
                    <div style={{ position: "relative" }} ref={doctorDropdownRef}>
                      ...
                      >
                        <span style={{ color: selectedDoctor ? T.text : T.textMuted }}>
                          {selectedDoctor ? (
                            <>
                              {selectedDoctor.name}
                              <span style={{ fontSize: "0.7rem", marginLeft: "6px", color: T.textMuted }}>
                            ...
                            {doctors.length > 0 ? (
                              doctors.map((doctor) => (
                                <button
                                  ...
                                  style={S.dropdownOption(formData.doctor_id === doctor.sys_user_id)}
                                >
                                  <div>
                                    <div>{doctor.name}</div>
                              ...
                              <div style={{ padding: "0.75rem", textAlign: "center", color: T.textMuted }}>
                ...
                {message && (
                  <div style={S.messageBox(message.includes("successfully"))}>
