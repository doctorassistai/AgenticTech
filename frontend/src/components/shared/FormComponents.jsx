...
export const PostponedBanner = ({ info, gated = false, onOverride }) => {
  ...
  const { isFuture, newDate, originalDate, reason, postponeCount } = info;
      ...
      <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 2, flexWrap: "wrap" }}>
        <Box>
          ...
          <Typography sx={{ fontSize: 12.5, color: "#333", lineHeight: 1.5 }}>
            {isFuture ? (
              <>This procedure is postponed {postponeCount > 0 && <b>({postponeCount} {postponeCount === 1 ? 'time' : 'times'})</b>} and rescheduled to <b>{newDate || "an unspecified date"}</b>. The scheduled date has not yet arrived{gated ? " — editing is locked until then." : "."}</>
            ) : (
              <>This procedure was postponed {postponeCount > 0 && <b>({postponeCount} {postponeCount === 1 ? 'time' : 'times'})</b>}{originalDate ? <> from <b style={{ textDecoration: "line-through" }}>{originalDate}</b></> : ""} and is now due. Proceed as normal.</>
            )}
          </Typography>
          {reason && (
...
export const WasPostponedTag = ({ count }) => (
  <Box sx={{
    ...
    {count > 1 ? `Was Postponed (${count}x)` : "Was Postponed"}
  </Box>
);
