...
// New menu inserted before OT Booking. Contains three sub-tabs:
//   1) Clinical Summary   -> GET  hms/users/data/context/patient-summary/{patientId}
//   2) Specialty Summary  -> GET  hms/users/data/context/radiation-oncology-summary/{patientId}
//                             POST hms/users/ai-legacy/radiation-oncology-summary (Generate button)
//   3) Synoptic Report    -> GET  hms/users/data/context/synoptic-report/{patientId}
...
const SPECIALTY_SUMMARY_GET_URL = (patientId) => `${API_BASE_URL}hms/users/data/context/radiation-oncology-summary/${patientId}`;
const SPECIALTY_SUMMARY_POST_URL = `${API_BASE_URL}hms/users/ai-legacy/radiation-oncology-summary`;
...
// ─────────────────────────────────────────────────────────────────────────────
// SUB-TAB 2 — SPECIALTY SUMMARY
// ─────────────────────────────────────────────────────────────────────────────
const SpecialtySummaryPanel = ({ patientId, doctorId }) => {
  const [loading, setLoading] = useState(false);
  ...
  const [consultationText, setConsultationText] = useState("Patient presenting for Radiation planning consult");

  const fetchSpecialtySummary = useCallback(async () => {
    if (!patientId) return;
    setLoading(true);
    setError("");
    try {
      const res = await fetch(SPECIALTY_SUMMARY_GET_URL(patientId));
      const data = await res.json();
      if (res.status === 404) {
        setSummary(null);
        setGeneratedAt(null);
      ...
      if (!res.ok || data?.status !== "success") {
        throw new Error(data?.detail || "Failed to fetch specialty summary");
      }
      setSummary(data?.data?.summary || null);
      setGeneratedAt(data?.data?.generated_at || null);
    } catch (err) {
      console.error("[SpecialtySummaryPanel] fetch error:", err);
      setError(err.message || "Unable to load specialty summary");
      setSummary(null);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => { fetchSpecialtySummary(); }, [fetchSpecialtySummary]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch(SPECIALTY_SUMMARY_POST_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.detail || "Failed to generate specialty summary");

      // Re-fetch from DB after successful generation
      await fetchSpecialtySummary();
    } catch (err) {
      console.error("[SpecialtySummaryPanel] generate error:", err);
      setGenError(err.message || "Failed to generate specialty summary");
    } finally {
      setGenerating(false);
    }
  };

  const diagnosisConfirmed = summary?.confirmed_diagnosis_present;

  return (
    <Box>
      <SectionBox title="Generate Specialty Summary">
      ...
      <SectionBox
        ...
        right={
          <Button size="small" onClick={fetchSpecialtySummary} disabled={loading}
            sx={{ ...outlineBtnSx, mt: 0, py: 0.4, px: 1.25, fontSize: 10 }}>
            <RefreshRounded sx={{ mr: 0.5, fontSize: 13 }} /> Refresh
          </Button>
        }
      >
        {!patientId && <EmptyState message="Enter a Patient ID to load the specialty summary." />}
        {patientId && loading && <LoadingState message="Fetching specialty summary…" />}
        {patientId && !loading && error && (
          <Typography sx={{ fontFamily: FONT, fontSize: 12, color: "#b00020" }}>{error}</Typography>
        )}
        {patientId && !loading && !error && !summary && (
          <EmptyState message="No specialty summary generated yet. Use Generate Summary above." />
        )}
        {patientId && !loading && !error && summary && (
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, flexWrap: "wrap" }}>
              <Typography sx={{ fontFamily: FONT, fontSize: 16, fontWeight: FW_NORMAL, color: C.textPrimary }}>
                {renderMarkdownBold(summary.diagnosis_header)}
              </Typography>
              <StatusChip
                label={diagnosisConfirmed ? "Diagnosis Confirmed" : "Diagnosis Unconfirmed"}
                tone={diagnosisConfirmed ? "positive" : "negative"}
              />
            </Box>
            ...
            <Box>
              ...
              <Typography sx={{ fontFamily: FONT, fontSize: 13, fontWeight: FW_LIGHT, lineHeight: 1.75, color: C.textPrimary }}>
                {renderMarkdownBold(summary.narrative)}
              </Typography>
            </Box>
...
// ─────────────────────────────────────────────────────────────────────────────
// SUB-TAB 3 — SYNOPTIC REPORT
// ─────────────────────────────────────────────────────────────────────────────
//
// Behavior requested: clicking "Generate Synoptic Report" calls the POST
// generate endpoint, and ONLY on success does it call the GET retrieve
// endpoint to populate the panel — mirroring SpecialtySummaryPanel's
// handleGenerate -> fetchSpecialtySummary() flow above.
//
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN — ClinicalSummaryTab (menu with 3 sub-tabs)
// ─────────────────────────────────────────────────────────────────────────────
const ClinicalSummaryTab = ({ patientId, doctorId }) => {
  const [sub, setSub] = useState(0);
  const SUBS = ["Clinical Summary", "Specialty Summary", "Synoptic Report"];
  ...
  return (
    <Box>
      ...
      {sub === 1 && <SpecialtySummaryPanel patientId={patientId} doctorId={doctorId} />}
      {sub === 2 && <SynopticSummaryPanel patientId={patientId} doctorId={doctorId} />}
    </Box>
  );
};
