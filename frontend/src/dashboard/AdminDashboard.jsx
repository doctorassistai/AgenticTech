...
const HEALTH_SERVICES = [
  ...
  { name: "Users", url: "hms/users/data/health" },
  { name: "AI Service", url: "hms/users/ai/health" },
  { name: "Audit Service", url: "hms/users/auditservice/health" },
  { name: "Workflow Engine", url: "hms/users/workflowengine/health" },
  { name: "Common Service", url: "hms/users/cm/health" },
];
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchServerTime = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}hms/admin/server-time`,
          { credentials: "include" }
        );
        ...
        setServerTime(new Date(data.server_time_utc));
      } catch (e) {
    ...
    intervalId = setInterval(() => {
      ...
      setServerTime((prev) =>
        prev ? new Date(prev.getTime() + 1000) : prev
      );
    }, 1000);
  ...
  //   const WS_URL = `${WS_PROTOCOL}://${window.location.host}/apihms/users/livelogs/ws/logs`;
  ...
  const TimeBar = () => (
    <div style={styles.timeBar}>
      <div>
        <strong>Server (UTC):</strong>{" "}
        {serverTime ? serverTime.toISOString().replace("T", " ").slice(0, 19) : "—"}
      </div>
      <div>
